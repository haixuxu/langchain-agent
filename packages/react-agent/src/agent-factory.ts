...
import { ReActTool, convertMCPToolToReActTool } from "./tool-converter.js";
...
/**
 * 创建不使用 function calling 的 ReAct Agent
 */
export async function createReActAgentWithMCPTools(
  config: MCPConfig,
  ...
  for (const serverConfig of config.mcpServers) {
    try {
      ...
      const serverTools = mcpTools.map((tool) =>
        convertMCPToolToReActTool(tool, client, serverConfig.name)
      );
  ...
  // 创建 ReAct Agent（不使用 function calling）
