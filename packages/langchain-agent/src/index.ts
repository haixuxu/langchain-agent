...
async function main() {
  try {
    ...
    const { agent, clients, cleanup, tools, confirmationManager } =
      await createAgentWithMCPTools(config, {
    ...
    // 启动REPL（传递确认管理器）
    await startREPL(agent, config, clients, tools, confirmationManager);
  } catch (error) {
    console.error("\n❌ 程序启动失败:");
